...
class MiniGPTCaption(nn.Module):
    def __init__(self, vocab_size, max_len, d_model=256, nhead=1, num_layers=1):
        super(MiniGPTCaption, self).__init__()
        ...
        self.sen_fc = nn.Linear(d_model, d_model)
        self.tanh = nn.Tanh()
    ...
    def forward(self, captions, caption_lengths):
        ...
        # Apply fully connected layer for output predictions
        output_logits = self.tanh(self.fc(transformer_output.permute(1, 0, 2)))
        sentence_embed = self.tanh(self.sen_fc(torch.mean(output_logits.permute(0, 2, 1), dim=2)))
        # print(output_logits.shape)
        return output_logits.permute(0, 2, 1), sentence_embed  # Reshape to (batch_size, vocab_size, max_len)
...
class CaptionTransformer(nn.Module):
    ...
    def forward(self, captions, caption_lengths):
        ...
        return transformer_output.permute(1, 2, 0), transformer_output.permute(1, 0, 2).mean(dim=1)
...
#         self.sentence_mlp = nn.Sequential(nn.Linear(d_model, d_model), nn.ReLU(), nn.Linear(d_model, d_model))
#         self.tanh = nn.Tanh()
...
#         transformer_output = self.tanh(transformer_output)
#         sentence_embed = self.sentence_mlp(transformer_output.mean(dim=2))
        
#         return transformer_output, transformer_output.mean(dim=2)
